    ...
    FloatingWindow {
        ...
        Rectangle {
            ...
            Flea.PickerFooter {
                ...
                edge: win.edge
                message: win.message.length > 0 ? win.message : Picker.statusLine(win.marks.length, Picker.totalBytes(win.marks))
                error: win.messageError
                hint: win.backendUnavailable ? "Esc cancel" : Picker.hints(win.req)
            }
