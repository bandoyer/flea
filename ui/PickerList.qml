...
ListView {
    ...
    Keys.onPressed: function (event) {
        ...
        if (event.key === Qt.Key_Escape) {
            ...
            root.picker.requestSort(Sort.nextOrder(Picker.SORT_ORDERS, root.backend.sortBy))
        } else if (action === "sortReverse") {
            root.picker.requestSort(Sort.reverseOrder(root.backend.sortBy, root.backend.sortDesc))
